...
```bash
export MARKET_RECAP_GITHUB_URL="https://github.com/<owner>/<repo>.git"
...
```bash
export MARKET_RECAP_GITHUB_URL="https://github.com/shiningwhite-cmd/market_replay_data.git"
export GITHUB_TOKEN="github_pat_xxx"
export MARKET_RECAP_BRANCH="main"
export MARKET_RECAP_TARGET_DIR="recaps"
export MARKET_RECAP_REPO_DIR="/tmp/market-recap-sync-repo"
export GIT_AUTHOR_NAME="market-recap-bot"
export GIT_AUTHOR_EMAIL="market-recap-bot@local"
```
...
- 若目标分支不存在（例如新仓库首次同步），脚本会自动初始化本地分支并完成首次 push。
- 若仓库完全空且没有默认分支，建议仍先在 GitHub 创建一次初始提交，以便权限与分支保护规则更直观生效。
